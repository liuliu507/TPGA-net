...
# from CPGA import CPGA
from TGF import TaskGuidedFusion
...
class pyCNN(nn.Module):
    def __init__(self, Classes, FM=64, NC=30, para_tune=True, use_task_guided_fusion=True):
        ...
        self.fusion = nn.Sequential(
            nn.Conv2d(cpt_embed * 2, cpt_embed * 2, 1),
            nn.BatchNorm2d(cpt_embed * 2),
            nn.LeakyReLU(),
            nn.Conv2d(cpt_embed * 2, cpt_embed, 1),
            nn.BatchNorm2d(cpt_embed),
            nn.LeakyReLU()
        )

        # 添加通道注意力机制，用于动态调整融合权重
        self.channel_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(cpt_embed * 2, cpt_embed // 4, 1),
            nn.LeakyReLU(),
            nn.Conv2d(cpt_embed // 4, cpt_embed * 2, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x1, x2, target=None):
        ...
        # 确保x2的空间维度与x1p_norm匹配
        x2p = self.proj_x2(x2)
        x2p = F.interpolate(x2p, size=(x1p_norm.size(2), x1p_norm.size(3)), mode='bilinear', align_corners=False)

        # 使用通道注意力机制融合特征
        fused_features = torch.cat([x1p_norm, x2p], dim=1)
        attention = self.channel_attention(fused_features)
        fused_features = fused_features * attention
        cpt_feat = self.fusion(fused_features)
