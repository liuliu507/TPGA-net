...
from pymodel_fuse_noGate_noTGF import pyCNN
# from pymodel_base import pyCNN


# ==================== 设置实验参数 ====================
DATASET_NAME = "Trento"  # 数据集名称: "Trento", "Muufl", "Houston", "Augsburg"
...
patchsize1 = 9
patchsize2 = 9
half_patch1 = patchsize1 // 2  # 改为使用整数除法
...
def train(train_loader, epochs, Classes, NC, device):
    """训练模型"""
    # 初始化模型
    para_tune = True
    # 学生模型
    cnn = pyCNN(FM=FM, NC=NC, Classes=Classes, para_tune=para_tune)   #pyCNN就是整个框架（有baseline版、和加上创新点之后的版本）
    cnn.to(device)
...
def run_single_experiment(dataset_name=DATASET_NAME, train_num=TRAIN_NUM, seed=42):
    """运行单次实验"""
    print(f"\n{'=' * 70}")
    ...
    net = train(train_loader, EPOCH, Classes, NC, device)
